...
try_seq_inbred <- function(input.seq,mrk,tol=10E-2,pos= NULL,verbose=FALSE)
{
  ...
  error=get(input.seq$data.name, pos=1)$error[try.ord + rep(c(0:(get(input.seq$data.name)$n.ind-1))*get(input.seq$data.name)$n.mar, each=length(try.ord)),],
                            rf.vec=rf.temp,
                            verbose=FALSE,
                            tol=tol)
  ...
  for(i in 1:(length(input.seq$seq.num)-1))
  {
    ...
    error=get(input.seq$data.name, pos=1)$error[try.ord[i+1,] + rep(c(0:(get(input.seq$data.name)$n.ind-1))*get(input.seq$data.name)$n.mar, each=length(try.ord[i+1,])),],
                              rf.vec=rf.temp,
                              verbose=FALSE,
                              tol=tol)
  ...
  error=get(input.seq$data.name, pos=1)$error[try.ord[length(input.seq$seq.num)+1,] + rep(c(0:(get(input.seq$data.name)$n.ind-1))*get(input.seq$data.name)$n.mar, each=length(try.ord[length(input.seq$seq.num)+1,])),],
                            rf.vec=rf.temp,
                            verbose=FALSE,
                            tol=tol)
...
### positioning before the given sequence
	...
	error=get(input.seq$data.name, pos=1)$error[c(mrk,input.seq$seq.num) + rep(c(0:(get(input.seq$data.name)$n.ind-1))*get(input.seq$data.name)$n.mar, each=length(c(mrk,input.seq$seq.num))),],
...
### positioning between markers of the given sequence
    for(i in 1:(length(input.seq$seq.num)-1)) {
	    ...
	    error=get(input.seq$data.name, pos=1)$error[c(input.seq$seq.num[1:i], mrk, input.seq$seq.num[(i+1):length(input.seq$seq.num)]) + rep(c(0:(get(input.seq$data.name)$n.ind-1))*get(input.seq$data.name)$n.mar, each=length(c(input.seq$seq.num[1:i], mrk, input.seq$seq.num[(i+1):length(input.seq$seq.num)]))),],
...
### positioning after the given sequence
	...
	error=get(input.seq$data.name, pos=1)$error[c(input.seq$seq.num,mrk) + rep(c(0:(get(input.seq$data.name)$n.ind-1))*get(input.seq$data.name)$n.mar, each=length(c(input.seq$seq.num,mrk))),],
