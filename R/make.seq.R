...
##'
##'
##' @param input.obj an object of class \code{onemap}, \code{rf.2pts},
...
##' which markers comprise the sequence. For an object \code{rf.2pts},
...
##' For an \code{rf.2pts} or \code{onemap} object, \code{phase} can be a
...
make.seq <-
  function(input.obj, arg = NULL, phase = NULL, data.name = NULL, twopt = NULL) {
    ...
    if(all(is.na(match(class(input.obj),c("onemap", "rf.2pts", "group", "compare", "try", "order")))))
      stop(deparse(substitute(input.obj))," is not an object of class 'onemap', 'rf.2pts', 'group', 'compare', 'try' or 'order'")
