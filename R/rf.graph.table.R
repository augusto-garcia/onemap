...
rf.graph.table <- function(input.seq, scale=1, axis.cex=1, main=NULL, inter=TRUE){
    if(is(get(input.seq$data.name), "outcross"))
        return(rf_graph_table_outcross(input.seq=input.seq,
                                       scale=scale,
                                       axis.cex=axis.cex,
                                       main=main,
                                       inter=inter))
...
rf_graph_table_outcross <- function(input.seq, scale=1, axis.cex=1, main=NULL, inter=TRUE)
{
    ...
    LOD <- lapply(get(input.seq$twopt)$analysis, function(x,i) as.matrix(as.dist(x[i,i], diag=TRUE, upper=TRUE)), input.seq$seq.num)
    mat<-t(get_mat_rf_out(input.seq, LOD=TRUE))
    mat.rf<-t(get_mat_rf_out(input.seq, LOD=FALSE))
    ...
    if(inter==FALSE)
        ...
        draw.rf.inter(info.graph=info.graph,scale=scale,cex=axis.cex)
}
...
{
     layout(matrix(1:2,1,2), widths = c(10,1))
    params <- par(bg="white", plt=c(0.1,.95, 0.1, 0.9),xpd=TRUE)
    y.adj<- .8/(-2+info.graph$n*2)
    ##print(info.graph$mat)
    if(is.null(main))
        main="LOD (above diag.) and Recombination Fraction Matrix"
    image(info.graph$mat, axes=FALSE,
          col=rainbow(n=500, start=min(info.graph$mat,na.rm=TRUE)*1.3,
                      end=max(info.graph$mat,na.rm=TRUE)*1.3), useRaster=TRUE)
    ...
    text(y=x, x=y.adj+rep(-diff(x)[1],info.graph$n), info.graph$seq.num, cex=cex, adj=1)
     #par(cex.axis=.8)
     #plot(x=rep(1,51), y=seq(from=0, to=.5, by=0.01), xlim=c(1,1.5), col=rainbow(51, start=0, end=.65), pch=15, axes=FALSE, xlab="", ylab="")
     #axis(4, pos=c(1.1,0))
    par(params)
}
...
draw.rf.inter<-function(info.graph, scale, cex){
    ...
    plotFunction <- function()
    {
        ...
        image(info.graph$mat, axes=FALSE, main="",
              col=rainbow(n=500, start=min(info.graph$mat,na.rm=TRUE)*1.3, end=max(info.graph$mat,na.rm=TRUE)*1.3), useRaster=TRUE)
