...
##'  draw_haplotypes(map1, map2, map3, ind = 1)
##'  
...
draw_haplotypes <- function(..., 
  ...
  if(is(get(input.map[[1]]$data.name), "outcross") | is(get(input.map[[1]]$data.name), "f2")){
    ...
    probs <- probs %>%
      ...
      mutate(H1_P1 = V1,
             ...
             H2_P1 = V1 ,
             H2_Het = V2) %>% 
      select(ind, grp, pos, pos2, H1_P1, H1_Het, H2_P1, H2_Het) %>% 
      gather(allele, prob, H1_P1, H1_Het, H2_P1, H2_Het) %>%
      mutate(homolog = factor(if_else(allele %in% c("H1_P1","H1_Het"), "H1", "H2"), levels = c("H2","H1")),
             allele = factor(if_else(allele %in% c("H1_P1","H2_P1"), "P1", "Heterozygote"), levels = c("Heterozygote","P1")),
             allele2 = factor(if_else(allele == "P1", "Homozygote", "Heterozygote" ), levels = c("Heterozygote","Homozygote")))
  }
  ...
  if(position == "split"){
    p <- p + geom_line(aes(x = pos2, y = allele2), size = ifelse(show.markers, 4, 5))
    if(show.markers) p <- p + geom_point(aes(y = allele2), size = 5, stroke = 2, na.rm = T, shape = "|")
  } 
  return(p)
}
