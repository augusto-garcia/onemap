...
plot.bc.onemap <- function(x) {
    ...
    if (suppressWarnings(all(levels(df.BC$value) == c("0", "1", "2")))) 
        labels.bc <- c("-", "AA", "AB") else if (all(levels(df.BC$value) == c("1", "2"))) 
        labels.bc <- c("AA", "AB")
    # Plotting
    g <- ggplot(data = df.BC, aes(x = ind, y = variable, fill = factor(value)))
    g <- g + geom_tile()
    g <- g + xlab("Individual") + ylab("Marker") + scale_fill_manual(name = "Genotype", 
        labels = labels.bc, values = c("#F21A00", "#3B9AB2", "#EBCC2A"))
    if (x$n.mar > 20) 
...
plot.riself.onemap <- function(x) {
    ...
    # Defining the label for genotypes
    if (suppressWarnings(all(levels(df.RIL$value) == c("0", "1", "2")))) 
        labels.ril <- c("-", "AA", "BB") else if (all(levels(df.RIL$value) == c("1", "2"))) 
        labels.ril <- c("AA", "BB")
    # Plotting
    g <- ggplot(data = df.RIL, aes(x = ind, y = variable, fill = factor(value)))
    g <- g + geom_tile()
    g <- g + xlab("Individual") + ylab("Marker") + scale_fill_manual(name = "Genotype", 
        labels = labels.ril, values = c("#F21A00", "#3B9AB2", "#EBCC2A"))
    if (x$n.mar > 20) 
        g <- g + theme(axis.text.y = element_blank(), axis.ticks.y = element_blank())
    return(g)
}
...
plot.f2.onemap <- function(x) {
    ...
    # Defining the label for genotypes
    if (suppressWarnings(all(levels(df.F2$value) == c("0", "1", "2", "3", 
        "4", "5")))) 
        labels.f2 <- c("-", "AA", "AB", "BB", "not BB", "not AA") else if (suppressWarnings(all(levels(df.F2$value) == c("1", "2", "3", 
        "4", "5")))) 
        labels.f2 <- c("AA", "AB", "BB", "not BB", "not AA") else if (suppressWarnings(all(levels(df.F2$value) == c("0", "1", "2", 
        "3")))) 
        labels.f2 <- c("-", "AA", "AB", "BB") else if (suppressWarnings(all(levels(df.F2$value) == c("1", "2", "3")))) 
        labels.f2 <- c("AA", "AB", "BB") else if (suppressWarnings(all(levels(df.F2$value) == c("0", "1", "2", 
        "3", "5")))) 
        labels.f2 <- c("-", "AA", "AB", "BB", "not AA") else if (suppressWarnings(all(levels(df.F2$value) == c("1", "2", "3", 
        "5")))) 
        labels.f2 <- c("AA", "AB", "BB", "not AA") else if (suppressWarnings(all(levels(df.F2$value) == c("0", "1", "2", 
        "3", "4")))) 
        labels.f2 <- c("-", "AA", "AB", "BB", "not BB") else if (suppressWarnings(all(levels(df.F2$value) == c("1", "2", "3", 
        "4")))) 
        labels.f2 <- c("AA", "AB", "BB", "not BB") else if (suppressWarnings(all(levels(df.F2$value) == c("0", "4", "5")))) 
        labels.f2 <- c("-", "not BB", "not AA") else if (suppressWarnings(all(levels(df.F2$value) == c("4", "5")))) 
        labels.f2 <- c("not BB", "not AA")
    # Plotting
    g <- ggplot(data = df.F2, aes(x = ind, y = variable, fill = factor(value)))
    g <- g + geom_tile()
    g <- g + xlab("Individual") + ylab("Marker") + scale_fill_manual(name = "Genotype", 
        labels = labels.f2, values = c("#000000", "#ECCBAE", "#046C9A", 
            "#D69C4E", "#85D4E3", "#74A089"))
    if (x$n.mar > 20) 
        g <- g + theme(axis.text.y = element_blank(), axis.ticks.y = element_blank())
    return(g)
}
...
create.dataframe.outcross.plot <- function(x) {
    ...
    for (i in 1:4) {
        if (length(which(x$segr.type == paste("A.", i, sep = "")) != 0)) {
            F1.A <- data.frame(x$geno[, which(x$segr.type == paste("A.", 
                i, sep = ""))])
            colnames(F1.A) <- colnames(x$geno)[which(x$segr.type == paste("A.", 
                i, sep = ""))]
            F1.A <- reshape2::melt(F1.A)
            F1.A <- cbind(ind = 1:x$n.ind, F1.A, Mk.type = paste("A.", 
                i, sep = ""))
            F1.A$value <- factor(F1.A$value)
            if (!exists("F1.A.")) 
                F1.A. <- data.frame(F1.A) else F1.A. <- rbind(F1.A., F1.A)
        }
    }
    class(F1.A.) <- "data.frame"
    # Markers of B type
    for (i in 1:3) {
        if (length(which(x$segr.type == paste("B", i, ".", i + 4, sep = "")) != 
            0)) {
            F1.B <- data.frame(x$geno[, which(x$segr.type == paste("B", 
                i, ".", i + 4, sep = ""))])
            colnames(F1.B) <- colnames(x$geno)[which(x$segr.type == paste("B", 
                i, ".", i + 4, sep = ""))]
            F1.B <- reshape2::melt(F1.B)
            F1.B <- cbind(ind = 1:x$n.ind, F1.B, Mk.type = paste("B", i, 
                ".", i + 4, sep = ""))
            F1.B$value <- factor(F1.B$value)
            if (!exists("F1.B.")) 
                F1.B. <- data.frame(F1.B) else F1.B. <- rbind(F1.B., F1.B)
        }
    }
    # Markers of C type
    for (i in 8) {
        if (length(which(x$segr.type == paste("C.", i, sep = "")) != 0)) {
            F1.C <- data.frame(x$geno[, which(x$segr.type == paste("C.", 
                i, sep = ""))])
            colnames(F1.C) <- colnames(x$geno)[which(x$segr.type == paste("C.", 
                i, sep = ""))]
            F1.C <- reshape2::melt(F1.C)
            F1.C <- cbind(ind = 1:x$n.ind, F1.C, Mk.type = paste("C.", 
                i, sep = ""))
            F1.C$value <- factor(F1.C$value)
            if (!exists("F1.C.")) 
                F1.C. <- data.frame(F1.C) else F1.C. <- rbind(F1.C., F1.C)
        }
    }
    # Markers of D1 type
    for (i in 9:13) {
        if (length(which(x$segr.type == paste("D1.", i, sep = "")) != 0)) {
            F1.D1 <- data.frame(x$geno[, which(x$segr.type == paste("D1.", 
                i, sep = ""))])
            colnames(F1.D1) <- colnames(x$geno)[which(x$segr.type == paste("D1.", 
                i, sep = ""))]
            F1.D1 <- reshape2::melt(F1.D1)
            F1.D1 <- cbind(ind = 1:x$n.ind, F1.D1, Mk.type = paste("D1.", 
                i, sep = ""))
            F1.D1$value <- factor(F1.D1$value)
            if (!exists("F1.D1.")) 
                F1.D1. <- data.frame(F1.D1) else F1.D1. <- rbind(F1.D1., F1.D1)
        }
    }
    # Markers of D2 type
    for (i in 14:18) {
        if (length(which(x$segr.type == paste("D2.", i, sep = "")) != 0)) {
            F1.D2 <- data.frame(x$geno[, which(x$segr.type == paste("D2.", 
                i, sep = ""))])
            colnames(F1.D2) <- colnames(x$geno)[which(x$segr.type == paste("D2.", 
                i, sep = ""))]
            F1.D2 <- reshape2::melt(F1.D2)
            F1.D2 <- cbind(ind = 1:x$n.ind, F1.D2, Mk.type = paste("D2.", 
                i, sep = ""))
            F1.D2$value <- factor(F1.D2$value)
            if (!exists("F1.D2.")) 
                F1.D2. <- data.frame(F1.D2) else F1.D2. <- rbind(F1.D2., F1.D2)
        }
    }
...
plot.outcross <- function(x, all = TRUE) {
    ...
    if (length(levels(x$variable)) > 20) 
        g <- g + theme(axis.text.y = element_blank(), axis.ticks.y = element_blank())
    if (all == TRUE) 
        g else g <- g + facet_grid(. ~ Mk.type) + theme(axis.text.x = element_text(size = 5))
    return(g)
}
## 
