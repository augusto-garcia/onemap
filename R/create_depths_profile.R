...
create_depths_profile <- function(onemap.obj = NULL, 
  ...
  if (is(onemap.obj, c("f2 intercross", "f2 backcross")) & 
      is.null(f1)) 
    stop("You must define f1 argument for this cross type \n")
  if (is(onemap.obj, "outcross")) {
    idx.mks <- colnames(onemap.obj$geno)[which(!(onemap.obj$segr.type %in% 
                                                   c("B3.7", "D1.10", "D2.15")))]
    if (length(idx.mks) > 0) {
      idx.mks <- colnames(onemap.obj$geno)[which((onemap.obj$segr.type %in% 
                                                    c("B3.7", "D1.10", "D2.15")))]
  ...
  if (is.null(parent1) | is.null(parent2)) 
    stop("Parents ID must be defined.")
  depths <- extract_depth(vcfR.object = vcfR.object, onemap.object = onemap.obj, 
                          vcf.par, parent1, parent2, f1 = f1, recovering = recovering)
  p1 <- p2 <- vector()
  if (is(onemap.obj, "outcross")) {
  ...
  idx.parents <- which(colnames(vcfR.object@gt[, -1]) %in% 
                         id.parents)
  gts <- vcfR.object@gt[, -1] %>% strsplit(":") %>% sapply("[", 
                                                           1) %>% matrix(ncol = dim(vcfR.object@gt)[2] - 1)
  MKS <- vcfR.object@fix[, 3]
  if (any(MKS == "." | is.na(MKS))) 
    MKS <- paste0(vcfR.object@fix[, 1], "_", vcfR.object@fix[, 
                                                             2])
  ...
  if (is(onemap.obj, "outcross") | is(onemap.obj, "f2")) {
    parents <- data.frame(parents, A = NA, AB = NA, BA = NA, 
                          B = NA)
  } else {
    parents <- data.frame(parents, A = NA, AB = NA)
  }
  alt <- depths$oalt %>% data.frame(mks = depths$mks) %>% gather("ind", 
                                                                 "alt", -"mks")
  ref <- depths$oref %>% data.frame(mks = depths$mks) %>% gather("ind", 
                                                                 "ref", -"mks")
  progeny <- merge(alt, ref)
  gt <- as.data.frame(cbind(ind = rownames(onemap.obj$geno), 
                            onemap.obj$geno))
  ...
  if (length(grep(pattern = ",", data$gt.vcf)) > 0) {
  ...
  if (GTfrom == "onemap") {
    ...
    p <- data %>% ggplot(aes(x = ref, y = alt, color = gt.onemap.alt.ref)) + 
      geom_point(alpha = alpha) + labs(title = "Depths", 
                                       x = "ref", y = "alt", color = "Genotypes") + 
      scale_colour_manual(name = "Genotypes", 
                          values = colors) + guides(colour = guide_legend(override.aes = list(alpha = 1))) + 
