...
/*******************************************************************
 * BASED ON PARENT GENOTYPES, DECIDE ON MARKER SEGREGATION PATTERN *
 *******************************************************************/
int get_marker_type(char marker_type[MARKER_TYPE_LEN],
 ...
 **************************/
void vcf2raw(char **filename, char **out_filename, int *n_parent1, char **parent1,
	     int *n_parent2, char **parent2, double *min_class) {
  ...
  const char * const B3[GT_TYPES_LEN] = { "a", "b", "ab", "-", "-", "-", "-" };
  const char * const D_ref[GT_TYPES_LEN] = { "a", "-", "ab", "-", "-", "-", "-" };
  const char * const D_alt[GT_TYPES_LEN] = { "-", "a", "ab", "-", "-", "-", "-" };
  ...
  while ((record = bcf_sweep_fwd(in_vcf)) && marker_count < MAX_VARIANTS) {
	...
	// Convert to appropriate marker type: B3, D1 or D2
	char marker_type[MARKER_TYPE_LEN];
	int type = get_marker_type(marker_type, is_het_parent1, is_hom_ref_parent1, is_hom_alt_parent1,
