...
`example.out.raw` (available with `OneMap` and saved in the directory
...
object. The function used to import data is named `read.onemap`.
...
example.out<- read.onemap(system.file("example",package="onemap"),"example.out.raw")
```
