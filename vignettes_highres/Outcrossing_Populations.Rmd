...
```{r}
vcfR.object <- read.vcfR(system.file("extdata/simu_cod_out.vcf", package = "onemap"))
...
```{r}
LG2_test_map <- map_avoid_unlinked(input.seq = any_seq)
```
...
Now, we can convert the phased VCF to onemap_progeny_haplotypes object using function `vcf2progeny_haplotypes`. The position, the group names and individuals id will be according with the ones contained in VCF file. This function can take some time to run if you select too many individuals or groups.
...
This is the simulated dataset. If OneMap algorithm could get all information needed, it could reproduce this haplotypes exactly how they were in simulated data. Let's see how OneMap estimate these haplotypes:
...
See that, besides markers are not in their exactly position, the recombination break points are the same as the ones simulated. The marker position could only be exactly the same as the ones simulated if we have a very big (or maybe infinite) population size, but here we have only 200 individuals, but this is not a problem once we could reproduce the haplotypes.
