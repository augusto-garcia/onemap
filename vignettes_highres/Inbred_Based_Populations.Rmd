...
Once you run PedigreeSim (directly or using our `run_pedsim` function), you should have the output file `genotypes.dat`. To convert this to OneMap raw file, you just need to specify the cross type (`cross`), which ones are the parents (`parent1` and `parent2`). When simulating f2 intercross you also need to point in `f1` which is the F1 individual and if you want to include missing genotypes (`miss.perc`). Only cross types `outcross` and `f2 intercross` are supported by now.
...
We can see that the markers were assigned to only one linkage group with. 
This division isn't so trustful, mostly if you are also working with dominat markers. If you have reference genome information for some of your markers, you can separate the groups using it and `group_seq` function (further information in [`Using the recombinations and the reference genome informations`](##using-the-recombinations-and-the-reference-genome-informations)) to add the markers without reference genome information. We can also confirm the separation of linkage groups in the ordering procedure.
...
In this case, Seriation will return an expected error:
...
Ordered markers are presented in both axis. Hotter the colors lower the recombination fraction between  markers related in each cell. Good map orders have hot colors in the diagonal and it is gradually turns to blue at the superior left and inferior right corners. This pattern means that markers that have low recombination fractions are really positioned closelly in the map.
...
color scale varies from red (small distances and big LODs) to dark blue. You can also change que number of colors from `rainbow` palette with argument `n.colors`,
...
Here, `ug` and `mds` approaches are the ones that gave results more close to the expected pattern. We can also see that probably there are more than one group, because of the big gaps. Let's see the map generated by `ug` algorithm with more detals using interactive mode of `rf_graph_table` function:
...
Markers from `90` to `45` seems to form a separated group, excluding marker `23` (we will call this group LG1). Markers from `42` to `30`, including marker `46` and `53` (LG2). Markers from `47` to `86` show stronge evidence that constitute other separated group, including markers `39`, `50` and `56` (LG3).  

Let's separate them and order again using the same method:

```{r}
# New LG1 will be this separated group
pos45 <- which(LG1_ug_f2$seq.num == 45) # Find position of marker 45
mksLG1 <- LG1_ug_f2$seq.num[1:pos45] # From marker 90 to 45
mksLG1 <- mksLG1[mksLG1!=23] # Excluding marker 23

# LG2
pos42 <- which(LG1_ug_f2$seq.num == 42)
pos30 <- which(LG1_ug_f2$seq.num == 30)
mksLG2 <- LG1_ug_f2$seq.num[pos42:pos30]
mksLG2 <- c(mksLG2,46,53)

# LG3
pos47 <- which(LG1_ug_f2$seq.num == 47)
mk47.86 <- LG1_ug_f2$seq.num[pos47:length(LG1_ug_f2$seq.num)] # use the position to find the 47 marker and take all the markers from there to the end of sequence
mksLG3 <- c(mk47.86, 39, 50, 56) 
...
Note that markers `66` , `61` , `25`, `51`, `5`, `54`, `8`, `45`, `32`, `43`, `11`, `2`, `10`and `41` could not be safely mapped to a single
...
which places markers `66` , `61` , `25`, `51`, `5`, `54`, `8`, `45`, `32`, `43`, `11`, `2`, `10`and `41`into their most likely positions.
...
step could additionally map markers `51`, `5`, `54`, `8`, `2` and `10`, but this depends on the dataset. Let us continue our analysis
...
The second round of `try_seq` added markers `58`, `1`, `69`, `30`, `72` and `12` (try
...
```{r}
LG2_edit <- drop_marker(LG2_f2_final, 42) # removing marker 42
```
...
```{r}
(LG2_temp <- try_seq(input.seq = LG2_edit_map, mrk = 42))
```

The result show us that the best position for this marker (with higher LOD) is the 29. Let's now include the marker at this position using:

```{r}
LG2_f2_final <- make_seq(LG2_temp, 29)
...
We can see that in the second round of `try_seq` markers `81` and `48` were added (please, try it). A careful examination of the graphics can be a good source of information about how markers where placed.
...
The sixth window presented one alternative order that seems better than the current one. Let's try to substitute the current order by the presented by ripple. See that, instead of `59-78-28-76-80` we have `59-80-76-28-78`. Let's find this window and substitute the sequence:

```{r}
which(LG3_f2_final$seq.num == 59)
new_seq <- LG3_f2_final$seq.num
new_seq[6:10] <- c(59,80,76,28,78)
...
```{r}
LG3_edit_map <- map(LG3_edit_seq)

ripple_seq(LG3_edit_map)
```
...
In our example, all markers without genomic information grouped with all chromosomes, then this approach did not gave us more information about grouping and from here we could follow the same strategy did in [Ordering markers within linkage groups](#Ordering markers within linkage groups). 
...
```{r, eval=FALSE}
(any_seq_map <- onemap::map(LG3seq_f2))
```

**NOTE**: <span style="color:orange">(new!)</span> If your map population is F2 intecross and your sequence has many markers (more than 60), we suggest to speed up `map` using BatchMap parallelization approach. See section [`Speed up analysis with parallelization`](#speed-up-analysis-with-parallelization) for more information.
