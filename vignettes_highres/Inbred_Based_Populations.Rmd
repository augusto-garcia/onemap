...
We can see that the markers were assigned to two linkage groups with
57, 22 and 12 markers remained unlinked.
...
Linkage group 2 has 22 markers, so using function `compare` is
...
Note that markers `41`, `76`, `89`, `90` and `91` could not be safely mapped to a single
...
which places markers `41`, `76`, `89`, `90` and `91` into their most likely positions.
...
The method also provides diagnostics graphics and parameters to find outliers to help users to filter the dataset. You can find more informations in `MDSMap` [vignette](https://CRAN.R-project.org/package=MDSMap). Here we will show a simples example of how it can be used for a outcrossing dataset.
