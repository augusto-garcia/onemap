...
**NOTE**: <span style="color:orange">(new!)</span> If you are working with f2 intercross mapping population and have many markers (more than 60), we suggest to speed up `seriation`, `rcd`, `record` and `ug` using BatchMap parallelization approach. See section [`Speed up analysis with parallelization`](#speed-up-analysis-with-parallelization) for more information.
...
It is note the case of this example, but you can have an error like this:
...
This means that some marker is not considered linked by the multipoint approach. Remember that MDS or do not order the markers using all information available as the HMM chain uses. To solve that what we can do is remove this marker and repeat the analysis, argument `rm_unlinked = TRUE` will do this automatically for you.

**NOTE**: <span style="color:orange">(new!)</span> If you are working with f2 intercross mapping population and have many markers (more than 60), we suggest to speed up `mds` using BatchMap parallelization approach. See section [`Speed up analysis with parallelization`](#speed-up-analysis-with-parallelization) for more information.

By now, we ordered our group in several ways, but, which one result in the best order? We can check it plotting the recombination fraction matrix and see if the generated maps obey the colors patterns expected.
...
**NOTE**: <span style="color:orange">(new!)</span> If you are working with f2 intercross mapping population and have many markers (more than 60), we suggest to speed up `map` using BatchMap parallelization approach. See section [`Speed up analysis with parallelization`](#speed-up-analysis-with-parallelization) for more information.
...
LG3_edit_map <- onemap::map(LG3_edit)
...
The fifth window presented alternative orders that seems better than the current one. Let's try to substitute the current order by the best presented by ripple. See that, instead of `59-49-76-28-80` we have `49-59-80-76-28`. Let's find this window and substitute the sequence:

```{r}
idx <- which(LG3_f2_final$seq.num == 59) 
new_seq <- LG3_f2_final$seq.num
new_seq[idx:(idx+4)] <- c(49,59,80,76,28)
...
**NOTE**: <span style="color:orange">(new!)</span> If your map population is F2 intercross and your sequence has many markers (more than 60), we suggest to speed up `map` using BatchMap parallelization approach. See section [`Speed up analysis with parallelization`](#speed-up-analysis-with-parallelization) for more information.
