...
If you are working with biallelic markers, as SNPs and indels (only codominant markers A.H.B), in VCF (Variant Call Format) files, there are two ways of import information from `VCF` to `OneMap`:
...
The `onemap `object `vcf_example_f2` generated is exactly the same as generated by the `vcf2raw` function combined with the `read_onemap`.
