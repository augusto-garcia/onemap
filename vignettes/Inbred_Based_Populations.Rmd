...
CHR1_add58 <- make_seq(CHR1_add10_seq, 20) # marker 58 was placed at the same position as before
```
...
```{r, echo=FALSE, results='hide'}
CHR2_test_seq <- drop_marker(CHR2_frame, 20)
CHR2_test_map <- onemap::map(CHR2_test_seq)
CHR2_add20_seq <- try_seq(CHR2_test_map, 20)
CHR2_add20 <- make_seq(CHR2_add20_seq, 20) # marker 20 was placed at the same position as before
```
...
```{r, eval=FALSE}
rf_graph_table(CHR2_add20)
```

```{r, echo=FALSE}
rf_graph_table(CHR2_add20, inter = FALSE)
```
