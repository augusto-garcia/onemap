...
The method also provides diagnostics graphics and parameters to find outliers to help users to filter the dataset. You can find more informations in `MDSMap` [vignette](https://CRAN.R-project.org/package=MDSMap). 
...
# Simulating maps with PedigreeSim

To test out new features we usually simulates datasets. Here we make available the functions we use to do this in PedigreeSim software. First, `run_pedsim` will generate the files needed by PedigreeSim and run it. It is important to highlight that PedigreeSim have more options than the ones provided by this function. For more informations please see PedigreeSim software documentation.

```{r}
run_pedsim(chromosome = c("Chr1", "Chr10"), n.marker = c(30,50), 
           tot.size.cm = c(100,150), centromere = c(50, 75),
           n.ind = 200, mk.types = c("A.H.B"), n.types = c(80), 
...
After, we can convert the PedigreeSim output files to onemap raw format.
