...
loci within each type using funcion `plot.by.type`:
...
function `test.segregation` using as argument `OneMap` object of class
...
function `Bonferroni.alpha` shows the alpha value that should be considered
...
It is easy to see the proportion of markers that will be discard if
this criteria is applied by drawing a graphic using function OneMap's
version of the function `plot` for objects of class
`onemap.segreg.test`:
...
(I need to write a function to show which markers are distorted, and
to select them).
