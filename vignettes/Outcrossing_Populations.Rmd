...
The method also provides diagnostics graphics and parameters to find outliers to help users to filter the dataset. You can find more informations in `MDSMap` [vignette](https://cran.r-project.org/web/packages/MDSMap/vignettes/MDSMap.pdf). Here we will show a simple example of how it can be used for ordering a large group from an outcrossing population.
...
There is only a group in this data set, with total of 1809 markers.
...
Here we will use the Principal Curves method available in `MDSMap` package. For this, we set the `ispc` argument to `TRUE`. For more details about the method, please, check the `MDSMap` tutorial. The diagnostic plot generated by the package will be at your home directory or at the directory defined in `mds.graph.file`. Also, the file you specify in `out.file` have the format requested for input in `MDSMap` package, than, you can use it to perfom analysis directly there.

The `MDSMap` converts the recombination fraction obtained by multi-dimensional scaling to centimorgan using Haldane or Kosambi functions. But here we update this values with the multipoint approach implemented in OneMap. 
...
These are simulated data, with five percent of missing data. The correct order would be the name of the markers in crescent order (eg. M001, M002, M003). You can see that any of the methods order perfectly the markers. For MDS method, there are local invertions and for order_seq method the markers are ordered according with their information (D1.10 close of D1.10 and D2.15 close of D2.15).

The group has originally 100 cM, the incorrect order of the markers inflate it.
...
<img src="map_all.png" width="600" height="600">
