...
With the `onemap_read_vcfR` you can convert the object from the `vcfR` package directly to `onemap`. The `onemap_read_vcfR` function keeps chromosome and position information for each marker in the onemap object generated.
...
As described in the `vcfR` package vignette, RAM memory use is an important consideration when using `vcfR`. Depending on your dataset, the object created can be huge and occupy a lot of memory.

After, you can use the `onemap_read_vcfR` function to convert this object to `onemap` object. The parameters used are the `vcfR.object` (which was just created), the identification of each parent (here, you must define only one sample for each parent), and the cross-type.
...
Note that the conversion filter out markers which are not informative for the informed cross-type. For example, in outcrossing species, markers that have both parents homozygous (aa x bb) do not inform recombinations and are removed from the data set. Only markers types contained in the table at [Creating the data file](#creating-the-data-file) are kept in the onemap object. Function `onemap_read_vcfR` prints at the screen the reason why markers were filtered.
