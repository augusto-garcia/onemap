...
Once all linkage groups were obtained using both strategies, we can draw a map for each strategy using the function `draw_map`. Since version 2.1.1007, OneMap a new version of `draw_map`, called `draw_map2`. The new function draws elegant linkage groups, and presents new arguments to personalize your draw.
...
```{r, echo=TRUE, fig=TRUE}
draw_map2(LG1_final, LG2_final, LG3_final, data = bins_example)
```

The figure will be saved in your work directory with the default name `map.eps`. You can change the file name and extension specifing them in the argument `output`. To know which figure extension are available check the function help with `?draw_map2`.

Drawing map builded with reference genome and  recombinations informations

```{r, echo=TRUE, eval=FALSE}
draw_map2(CHR1_final, CHR2_final, CHR3_final, data = bins_example, output= "map_ref.pdf", 
          col.group = "#58A4B0",
         col.mark= "#335C81")
``` 

![](vignettes/map_ref.pdf)
...
```{r, width = 9.5, height = 9.5}
CHR1_comp <- list(LG1_final, CHR1_final)
draw_map2(LG1_final, CHR1_final, data=bins_example )
```
