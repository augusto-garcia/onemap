...
test_that("create depth profile", {
  test_depth_profile <- function(df, cross, parent1, parent2, n.genos, n.genos.alt.ref){
    eval(bquote(data <- read.vcfR(.(df))))
    eval(bquote(onemap.obj <- onemap_read_vcfR(data, cross = .(cross), parent1 = .(parent1), parent2 = .(parent2))))
    temp.file <- tempfile()
    eval(bquote(p <- create_depths_profile(onemap.obj, vcfR.object = data, 
